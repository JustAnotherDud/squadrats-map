...
- **PWA**: `manifest.json` (root, `start_url` = the hub, `display: standalone`,
  ...
  from one shape (maple leaf on Squadrats purple `#663399`). The `<link>` tags
  are in every page (via the two stub templates for the generated ones),
  `mapa.html` included for a consistent favicon; installing from anywhere still
  opens the hub (`start_url`). `mapa.html` stays out of `nav.js` and the hub as
...
- `atletas/`: one static profile page per club athlete, at
  ...
  region names linking to `regioes/<c|d>-<slug>.html`. `perfil.js` +
...
- `regioes/`: one static page per concelho/distrito **with activity** (~100
  ...
  `<key>.html` stubs are generated by `pipeline/gen_regiao_stubs.py` (in the
...
- **The athlete roster is hard-coded in 5 front-end spots**: `historico.html`
  (`ATLETAS`, `COR_FALLBACK`), `regioes/regiao.js` (`COR_FB`), `index.html`
  (`A`), `club.html` (`COR_ATLETA`, offline fallback only). The real source is
  `ATHLETES_JSON` (a secret the browser can't read); colours have a single
  runtime source in `membros_cores.json`, but the name lists and the offline
  colour fallbacks must be edited by hand if the club changes. `atletas/index.html`
  is the exception, it is regenerated by `gen_profile_stubs.py`. If someone
  leaves the club, their historical events/gains still render (grey, name from
  the data) but with a correct profile link (slugified, not from a stale map).
  Fixing this properly means deriving the roster from data the pages already
  fetch (`club.json` / `club_regioes.json` carry the names in bit order).
