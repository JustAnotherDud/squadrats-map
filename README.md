...
- `pipeline/`: fetches the map owner's squares and produces the files in
  ...
  - `refdata/`: municipality/district borders at classification precision
    ...
    (`mapa.html` never classifies with them). Both are regenerated by
  ...
  - `refdata/foreign_muni/<CC>.geojson`: fine (municipality) geometry, but
    **clipped to the area the club has already visited + 10 km**, not the
    ...
    `grid_totals.json` to match. Germany at full GADM level 4 is ~11 k
    Gemeinden / 20 MB for the 3 cities anyone has ridden; clipped it is
    ~200 KB. See "Foreign municipalities: clip to visited area" below.
...
`region=null` — the square shows up on `mapa.html` and `club.html` with a
...
1. **New country** (no `foreign_muni/<CC>.geojson` yet): download the country
   from **GADM 4.1** (`gadm.org` → level 4 GeoJSON, e.g.
   `gadm41_CHE_4.json`), reshape each feature to
   `properties = {country: "CH", region: <municipality name>, parent: <region name>}`
   (disambiguate duplicate names the way `Calheta (Açores)` /
   `Lagoa (Faro)` are), save as `pipeline/refdata/foreign_muni/CH.geojson`,
   then run the clip (step 3). Also add `refdata/foreign/CH.geojson` (level 2
   regions, same shape) if you want a region level, and both files to
   `prep.py`'s table.
2. **Same country, wider area than before:** nothing to download — the full
   file is gone, but the clip re-runs against whatever `foreign_muni/<CC>`
